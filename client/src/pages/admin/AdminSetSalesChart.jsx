...
const SET_NAMES = {
  ...
  'zhashyl-ömür':    'Zhashyl Ömür',    'jenil-ashkana':   'Jenil Ashkana',
  ...
  'uzak-koldon':     'Uzak Koldon',     'önügüü-set':      'Önügüü Set',
  'dayar-tütük':     'Dayar Tütük',     'nelikvid':        'Неликвид',
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            {grandRevenue > 0 && (
              <div style={{ background:'#e3f2fd', borderRadius:10, padding:'6px 14px', textAlign:'right' }}>
                ...
                <div style={{ fontSize:18, fontWeight:800, color:'#1565c0' }}>{grandRevenue.toLocaleString('ru')} с</div>
              </div>
            )}
          </div>
        )}
        ...
        <>
          ...
          {totals.length > 0 && (
            <div style={{ border:'1px solid #eee', borderRadius:10, overflow:'hidden', background:'#fff' }}>
              <div style={{ display:'grid', gridTemplateColumns:'24px 1fr 80px 100px 130px', padding:'8px 14px', background:'#f7f8fa', borderBottom:'1px solid #eee', fontSize:11, fontWeight:700, color:'#888', textTransform:'uppercase', letterSpacing:0.5 }}>
                <span>#</span><span>Товар</span><span style={{ textAlign:'right' }}>% от итога</span><span style={{ textAlign:'right' }}>Продано, шт</span><span style={{ textAlign:'right' }}>Сумма, с</span>
              </div>
              {totals.map((item,i) => {
                const pct = grandTotal>0 ? Math.round(item.total/grandTotal*100) : 0;
                return (
                  <div key={item.name} style={{ display:'grid', gridTemplateColumns:'24px 1fr 80px 100px 130px', padding:'9px 14px', borderBottom:'1px solid #f5f5f5', fontSize:13, alignItems:'center' }}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'24px 1fr 80px 100px 130px', padding:'10px 14px', background:'#f7f8fa', fontSize:13 }}>
                ...
                <span style={{ textAlign:'right', fontWeight:800, fontSize:15, color:'#1565c0' }}>{grandRevenue.toLocaleString('ru')} с</span>
              </div>
            </div>
          )}
        </>
