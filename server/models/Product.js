...
const productSchema = new mongoose.Schema({
  ...
  // Себестоимость живёт в своей валюте: у привозного товара это закупочная цена
  // поставщика (Оудэбао считает в юанях), у своего производства — сомы.
  costCurrency:      { type: String, enum: ['KGS', 'KZT', 'CNY', 'USD'], default: 'KGS' },
  ...
  // против «Эко мангал R 10»), поэтому связь держим на артикуле: он не меняется,
  // когда номенклатуру переименовывают. Заполняется сам при загрузке остатков,
  // если в выгрузке есть колонка «Артикул».
  skuByBase: {
  ...
  // Имя номенклатуры, каким оно пришло в последней выгрузке каждой базы.
  // Нужно для двух вещей. Первая: заметить, что в 1С позицию переименовали —
  // связь по артикулу такое переживает молча, и расхождение с карточкой копилось
  // незамеченным. Вторая: найти товар по прежнему имени, если карточку
  // переименовали у нас, а в 1С осталось старое название.
  nameByBase: {
    makein:   { type: String, default: '' },
    matkasym: { type: String, default: '' },
    tubes:    { type: String, default: '' },
  ...
  pendingReceive:   { type: Boolean, default: false },  // товар на складе, ожидает приёмки (подсчёта)
